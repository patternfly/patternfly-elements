...
{% band header="Interacting with our web components API" %}
  ...
  Finally, let’s call the `toggle` method on the pfe-accordion DOM API so we can open the first panel of the accordion when the page loads.
  So we need to update the `useEffect` callback.
