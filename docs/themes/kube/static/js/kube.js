...
(function(Kube)
{
    ...
    Kube.Offcanvas.prototype = {
    	...
    	closeAll: function()
    	{
    		...
    		if ($elms.length !== 0)
    		{
                $elms.each(function()
                {
                    ...
                    if ($el.hasClass('open'))
                    {
                        $el.css('width', '67%').animation('hide');
    	...
    	pullBody: function()
    	{
            if (this.opts.push)
            {
                $('body').animate({ width: '100%' }, 300, function() { $(this).removeClass('offcanvas-push-body'); });
            }
    	},
    	pushBody: function()
    	{
            if (this.opts.push)
            {
                var properties = (this.opts.direction === 'left') ? { 'width': '67%' } : { 'width': '-' + '67%' };
                $('body').addClass('offcanvas-push-body').animate(properties, 300);
            }
    	}
    };
