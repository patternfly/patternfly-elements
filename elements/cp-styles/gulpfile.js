...
gulp.task("replaceVarsStyles", () => {
  return gulp
    ...
    .pipe(
      replace(
        /<style class="document-style">[\s\S]*<\/style>/g,
        '<style class="document-style">' +
          fs.readFileSync("./cp-vars.css") +
          "</style>"
      )
    )
    .pipe(gulp.dest("./"));
...
gulp.task("replaceTypographyStyles", () => {
  return gulp
    .src("./cp-typography.js")
    .pipe(
      replace(
        /<style class="document-style">[\s\S]*<\/style>/g,
        '<style class="document-style">' +
          fs.readFileSync("./cp-typography.css") +
          "</style>"
      )
    )
    .pipe(gulp.dest("./"));
...
gulp.task("watch", () => {
  watcher = gulp.watch(
    ["./cp-styles.js", "./cp-vars.js", "./cp-typography.js", "./*.scss"],
    gulp.series(
      "stopwatch",
      "sass",
      "replaceStyles",
