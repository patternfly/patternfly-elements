...
export class PfV5AlertCloseEvent extends Event {
 ...
 */
@customElement('pf-v5-alert')
export class PfV5Alert extends LitElement {
  ...
  override render(): TemplateResult<1> {
    ...
    return html`
      ...
      <div id="icon">
        <slot name="icon">
          <pf-v5-icon icon="${icon}"></pf-v5-icon>
        </slot>
      </div>
  ...
  protected timeoutChanged(): void {
    ...
    if (timeout > 0) {
      this.#timeoutId = setTimeout(() => {
        if (this.isConnected && this.dispatchEvent(new PfV5AlertCloseEvent('timeout'))) {
          this.remove();
        }
  ...
  #onCloseClick() {
    if (this.isConnected && this.dispatchEvent(new PfV5AlertCloseEvent())) {
      clearTimeout(this.#timeoutId);
      this.remove();
    }
  }
