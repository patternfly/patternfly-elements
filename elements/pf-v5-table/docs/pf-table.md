...
{% band header="Usage" %}
  ...
  To make a table sortable, add the `sortable` attribute to `<pf-v5-th>` elements
  in the table header (`<pf-v5-thead>`). When the user clicks on a header, the 
  ...
  2. set the `compound-expand` attribute on each `<pf-v5-td>` in the row to a
     string that's unique to that cell
  3. slot the content for each expandable `<pf-v5-td>` into the row, into the slot
