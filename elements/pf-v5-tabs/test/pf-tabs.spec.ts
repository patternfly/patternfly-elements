...
import { PfV5Tabs } from '../pf-v5-tabs.js';
...
describe('<pf-v5-tabs>', function() {
  it('instantiates imperatively', function() {
    expect(document.createElement('pf-v5-tabs')).to.be.an.instanceof(PfV5Tabs);
    expect(document.createElement('pf-v5-tab')).to.be.an.instanceof(PfV5Tab);
    expect(document.createElement('pf-v5-tab-panel')).to.be.an.instanceof(PfV5TabPanel);
  });

  it('should upgrade', async function() {
    const el = await createFixture<PfV5Tabs>(html`<pf-v5-tabs></pf-v5-tabs>`);
    expect(el, 'pf-v5-tabs should be an instance of PfeTabs')
        .to.be.an.instanceOf(customElements.get('pf-v5-tabs'))
        .and
        .to.be.an.instanceOf(PfV5Tabs);
  });

  describe('with three valid tab pairs', function() {
    let element: PfV5Tabs;

    const updateComplete = () => allUpdates(element);

    beforeEach(async function() {
      element = await createFixture<PfV5Tabs>(html`
        <pf-v5-tabs>
          <pf-v5-tab slot="tab">tab-1</pf-v5-tab>
          <pf-v5-tab-panel>panel-1</pf-v5-tab-panel>
          <pf-v5-tab slot="tab">tab-2</pf-v5-tab>
          <pf-v5-tab-panel>panel-2</pf-v5-tab-panel>
          <pf-v5-tab slot="tab">tab-3</pf-v5-tab>
          <pf-v5-tab-panel>panel-3</pf-v5-tab-panel>
        </pf-v5-tabs>
      `);
    });
    ...
    describe('pressing Tab', function() {
      ...
      it('should activate the first focusable tab', function() {
        expect(element.querySelector('pf-v5-tab')).to.have.attribute('active');
      });

      it('should activate the first tab panel', function() {
        expect(element.querySelector('pf-v5-tab')).to.not.have.attribute('hidden');
      });
    });
    ...
    describe('setting the second tab\'s `active` attribute', function() {
      beforeEach(function() {
        const [, tab] = element.querySelectorAll('pf-v5-tab');
        tab!.active = true;
      });
      ...
      it('should deactivate previously active tab', function() {
        expect(element.querySelector('pf-v5-tab')).to.not.have.attribute('active');
      });
    ...
    describe('setting `activeIndex` to 2', function() {
      ...
      describe('then setting the first tab\'s `disabled` attribute', function() {
        beforeEach(async function() {
          element.querySelector('pf-v5-tab')!.disabled = true;
        });
        ...
        describe('and clicking the disabled tab', function() {
          beforeEach(async function() {
            element.querySelector('pf-v5-tab')!.click();
          });

          beforeEach(updateComplete);

          it('should not activate', function() {
            const [first] = element.querySelectorAll('pf-v5-tab');
    ...
    describe(`when navigated by keyboard`, function() {
      describe('when focused on the first tab', function() {
        beforeEach(async function() {
          element.querySelector('pf-v5-tab')!.focus();
        });
        ...
        describe('pressing ArrowRight', function() {
          ...
          it('should activate the next tab', function() {
            const [first, second, third] = element.querySelectorAll('pf-v5-tab');
            expect(first).to.not.have.attribute('active');
      ...
      describe('pressing Tab', function() {
        ...
        describe('pressing ArrowRight key', function() {
          ...
          it('should focus the second tab', function() {
            const [, second] = element.querySelectorAll('pf-v5-tab');
  ...
  describe('when no active tab is given and the first tab is disabled', function() {
    ...
    beforeEach(async function() {
      element = await createFixture<PfV5Tabs>(html`
        <pf-v5-tabs>
          <pf-v5-tab id="tab1" slot="tab" disabled></pf-v5-tab>
          <pf-v5-tab id="tab2" slot="tab"></pf-v5-tab>
          <pf-v5-tab id="tab3" slot="tab"></pf-v5-tab>
          <pf-v5-tab id="tab4" slot="tab" disabled></pf-v5-tab>
          <pf-v5-tab id="tab5" slot="tab" aria-disabled="true"></pf-v5-tab>
          <pf-v5-tab-panel></pf-v5-tab-panel>
          <pf-v5-tab-panel></pf-v5-tab-panel>
          <pf-v5-tab-panel></pf-v5-tab-panel>
          <pf-v5-tab-panel></pf-v5-tab-panel>
          <pf-v5-tab-panel></pf-v5-tab-panel>
