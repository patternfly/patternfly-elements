...
const e = { ENTER: 13, DOWN: 40, UP: 38, ESC: 27 };
let n = !1;
class i extends t {
  get html() {
  ...
  attributeChangedCallback(t, e, n) {
    ...
    let i = this.shadowRoot
        .querySelector("slot")
        .assignedNodes()
        .filter(t => t.nodeType === Node.ELEMENT_NODE)[0],
      s = this.shadowRoot.querySelector(".clear-search"),
      o = this.shadowRoot.querySelector(".search-button");
    switch (t) {
      case "loading":
        this.loading && "" !== i.value
      ...
      case "init-value":
        this["init-value"] !== n &&
          ((i.value = n),
          "" === n || this.isDisabled
            ? (o.setAttribute("disabled", ""), s.setAttribute("hidden", ""))
            : (o.removeAttribute("disabled"), s.removeAttribute("hidden")));
        break;
      case "is-disabled":
        this.isDisabled
  ...
  set selectedValue(t) {
    this.setAttribute("selected-value", t);
  }
  set isDisabled(t) {
    t
  ...
  set initValue(t) {
    this.setAttribute("init-value", t);
  }
  get debounce() {
  ...
  _inputChanged() {
    ...
    this._input.hasAttribute("disabled") ||
      ...
      !1 === n &&
        ((n = !0),
        window.setTimeout(() => {
          this._sendAutocompleteRequest(this._input.value), (n = !1);
        }, this.debounce));
  }
  _clear() {
    (this._input.value = ""),
  ...
  _openDroplist() {
    (this.activeIndex = null),
      this._dropdown.setAttribute("open", !0),
      this._dropdown.setAttribute("active-index", null);
  }
  _optionSelected(t) {
    let e = t.detail.optionValue;
    (this._input.value = e), this._doSearch(e);
  }
  _doSearch(t) {
    this.dispatchEvent(
      new CustomEvent("pfe-search-event", {
        detail: { searchValue: t },
        bubbles: !0,
        composed: !0
      })
    ),
  ...
  _autocompleteCallback(t) {
    (this._dropdown.data = t),
      (this._dropdown.reflow = !0),
      0 !== t.length ? this._openDroplist() : this._closeDroplist();
  }
  _reset() {
  ...
  _activeOption(t) {
    if (null !== t && "null" !== t)
      return this._dropdown.shadowRoot.querySelector(
        "li:nth-child(" + (parseInt(t, 10) + 1) + ")"
      ).innerHTML;
  }
  _inputKeyUp(t) {
    let n = t.keyCode;
    if (
      0 === this._dropdown.data.length &&
      n !== e.DOWN &&
      n !== e.UP &&
      n !== e.ENTER &&
      n !== e.ESC
      ...
      return;
    let i = this._dropdown.activeIndex,
      s = this._dropdown.data.length;
    if (n == e.ESC) this._closeDroplist();
    else if (n === e.UP) {
      if (!this._dropdown.open) return;
      (i = null === i || "null" === i ? s : parseInt(i, 10)),
        (i -= 1) < 0 && (i = s - 1),
        (this._input.value = this._activeOption(i));
    } else if (n === e.DOWN) {
      if (!this._dropdown.open) return;
      (i = null === i || "null" === i ? -1 : parseInt(i, 10)),
        (i += 1) > s - 1 && (i = 0),
        (this._input.value = this._activeOption(i));
    } else if (n === e.ENTER) {
      let t = this._input.value;
      return void this._doSearch(t);
    }
    null !== i && "null" !== i
      ? this._input.setAttribute("aria-activedescendant", "option-" + i)
      : this._input.setAttribute("aria-activedescendant", ""),
      (this.activeIndex = i),
      (this._dropdown.activeIndex = i);
  }
}
class s extends t {
  get html() {
    ...
    "LI" === t.target.tagName &&
      this.dispatchEvent(
        new CustomEvent("pfe-option-selected", {
          detail: { optionValue: t.target.innerText },
          bubbles: !0,
          composed: !0
        })
      );
  ...
  _renderOptions() {
    ...
    (this._ariaAnnounce.innerHTML = `There are ${
      ...
      (this._ul.innerHTML = `${t
        .map(
          (t, e) =>
            `<li id="option-${e}" role="option" tabindex="-1" value="${t}">${t}</li>`
        )
        .join("")}`);
  }
  static get observedAttributes() {
  ...
  attributeChangedCallback(t, e, n) {
    super.attributeChangedCallback(),
      ...
      "active-index" === t && e !== n && this._activeIndexChanged(),
      "reflow" === t && this._renderOptions();
  }
  _activeIndexChanged() {
    if (
      !this.data ||
  ...
  set activeIndex(t) {
    this.setAttribute("active-index", t);
  }
  get reflow() {
