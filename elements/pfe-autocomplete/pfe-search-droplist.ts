 ...
 * @csspart {HTMLULElement} item-index-${n} - Each item in in the dropdown receives a 0-indexed `item-index-${n}` part
 *                                            e.g. `item-index-0`, `item-index-1`
 *
...
export class PfeSearchDroplist extends LitElement {
  ...
  @property({ attribute: 'announce-template', reflect: true }) announceTemplate = '';
  ...
  @bound protected _renderOptions() {
    this.ariaAnnounceText =
      ...
      : this.announceTemplate.replace('${numOptions}', this.data.length.toString());
  }
