 ...
 *        ```
 *
 * @slot text Optionally override the text of the button.
 * @slot icon
...
export class PfeClipboard extends LitElement {
  ...
  @state() private textSlotDefault = 'Copy';

  private slots = new SlotController(this, ['icon', 'text', 'success-text',
    // deprecated
    'text--success',
  ]);
  ...
  render() {
    return html`
      ...
      ${!this.noIcon ? html`
          <div class="pfe-clipboard__icon">
            <slot name="icon" id="icon">
              ...
              <svg id="icon--copy" xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 32 32">
                <g></g>
      ...
      <div class="pfe-clipboard__text">
          <slot name="text" id="text">${this.textSlotDefault}</slot>
      </div>
      <div class="pfe-clipboard__text--success" role="alert">
          <slot name="success-text" id="success-text">Copied</slot>
