 ...
 */
  ...
  render() {
    const counterReset = this.codeLineNumberStart !== 1 ? `linenumber ${this.codeLineNumberStart - 1}` : null;
    return html`
      <slot></slot>
      <pre class="${this.appliedCssClass()}" style="${styleMap({ counterReset })}"><code codeblock-render class="${this.prismClassName}">${unsafeHTML(this.renderedCodeBlock)}</code></pre>
    `;
  }
  ...
  private appliedCssClass() {
    return this.prismClassName + this.lineNumberCssClass();
  }
