 ...
 *
 * @csspart pre - The `<pre>` container for the codeblock
 * @csspart code - The `<code>` element containing the rendered codeblock
 */
 ...
 */
  ...
  /** prism.js language css class for the snippet */
  private get prismClassName() {
    return `language-${this.codeLanguage}`;
  }

  /** prism.js grammar for the snippet */
  private get grammar() {
  ...
  render() {
    const { codeLineNumbers, codeLineNumberStart, prismClassName, renderedCodeBlock } = this;
    const counterReset = codeLineNumberStart !== 1 ? `linenumber ${codeLineNumberStart - 1}` : null;
    const content = unsafeHTML(renderedCodeBlock);
    return html`
      <pre part="pre"
           class="${prismClassName}${codeLineNumbers ? ' line-numbers' : ''}"
           style="${styleMap({ counterReset })}"><code
             part="code"
             class="${prismClassName}"
           >${content}</code></pre>
    `;
  }
  ...
  private updateCodeBlock() {
    this.renderedCodeBlock = Prism.highlight(this.codeblock, this.grammar, this.prismClassName);
