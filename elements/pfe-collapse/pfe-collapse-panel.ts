 ...
 * @fires pfe-collapse-panel:animation-start {@deprecated use `animation-start`}
 * @fires pfe-collapse-panel:animation-end {@deprecated use `animation-end`}
 */
...
export class PfeCollapsePanel extends LitElement {
  ...
  private _transitionEndHandler(event: TransitionEvent) {
    ...
    const { expanded } = this;
    this.dispatchEvent(deprecatedCustomEvent('pfe-collapse-panel:animation-end', { expanded, panel }));
    this.dispatchEvent(new AnimationEndEvent(this, expanded));
  }

  private _fireAnimationEvent(state: 'opening'|'closing') {
    this.dispatchEvent(deprecatedCustomEvent('pfe-collapse-panel:animation-start', { state, panel: this }));
