...
import { elementUpdated, assert } from "@open-wc/testing/index-no-side-effects.js";
...
describe("<pfe-content-set>", () => {
  ...
  it("should upgrade", async () => {
    ...
    assert.instanceOf(pfeContentSet, customElements.get("pfe-content-set"), "pfe-content-set should be an instance of PfeContentSet");
  });
  ...
  it("should be an accordion", async () => {
    ...
    const thirdPanel = pfeAccordion.querySelector("pfe-accordion-panel:nth-of-type(3)");

    assert.equal(firstHeader.getAttribute("aria-controls"), firstPanel.id);
    assert.isTrue(firstPanel.hasAttribute("aria-labelledby"));
    assert.isTrue(secondPanel.hasAttribute("aria-labelledby"));
    assert.equal(thirdHeader.getAttribute("aria-controls"), thirdPanel.id);
  });
  ...
  it("should upgrade successfully with nested tabs", async () => {
  ...
  it("should set the correct \"on\" attribute from a parent component that has a color attribute to the nested tabs",
    () => {
  ...
  it("should set the correct \"on\" attribute from a parent component that has a color attribute to the nested accordion",
    () => {
      Promise.all([customElements.whenDefined("pfe-content-set")]).then(() => {
