 ...
 * @csspart button - The dropdown's toggle button
 * @csspart menu - The dropdown's menu
 *
...
export class PfeDropdown extends LitElement {
  ...
  render() {
    return html`
      <div class="pfe-dropdown__container" part="container">
        <!-- toggle -->
  ...
  private async _selectItem(item: PfeDropdownItem, type?: PfeDropdownOption['type']) {
    if (type === 'action') {
      ...
      this.dispatchEvent(deprecatedCustomEvent('pfe-dropdown:change', { action }));
