...
describe('<pfe-jump-links>', function() {
  ...
  it(`should fire a change when makeActive() is called`, async function() {
    ...
    const { index } = await oneEvent(element, 'change') as unknown as ChangeEvent;
  ...
  it(`should fire a content-change when panel contents change`, async function() {
    ...
    addChildNode();
    await oneEvent(panel, 'content-change');

    // Ensure @deprecated `pfe-jump-links-panel:change` fires
    addChildNode();
    const event2 = await oneEvent(panel, 'pfe-jump-links-panel:change');
