...
export class PfePrimaryDetail extends LitElement {
        ...
        <div id="details-wrapper">
          <div id="details-wrapper__header">
            <button
                ...
                aria-expanded="${ifDefined(this.breakpoint !== 'compact' ? undefined : String(this.active))}"
                @click="${this.closeAll}">
  ...
  @bound private _setBreakpoint() {
    ...
    for (const toggle of this.querySelectorAll('[slot$="nav"]')) {
      toggle.removeAttribute(this.breakpoint === 'desktop' ? 'aria-expanded' : 'aria-selected');
