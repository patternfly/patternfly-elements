...
class RhNumber extends HTMLElement {
    ...
    _setInitialNumber() {
        const initialNumber = typeof this.number === 'undefined' ? this.textContent : this.number;
    ...
    _numberAttrUpdate(oldVal, newVal) {
        this.shadowRoot.querySelector('span').textContent = numeral(newVal).format(this.getAttribute('format'));
    }
}
