...
const getContent = (name: string, { height, width, path, xOffset = 0, yOffset = 0 }: IconSpec) =>
  `import { svg } from 'lit';
export default svg\`<svg xmlns="http://www.w3.org/2000/svg" data-icon-name="${name}" height="${height}" width="${width}" viewBox="${[xOffset, yOffset, width, height].join(' ')}"><path d="${path}" /></svg>\`;`;

for (const [setName, icons] of Object.entries(SETS)) {
  const dir = new URL(`../../elements/pfe-icon/icons/${setName}/`, import.meta.url);
