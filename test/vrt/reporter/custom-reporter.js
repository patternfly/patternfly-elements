...
var util = require("util"),
    ...
    fs = require("fs"),
    path = require("path"),
    sanitizeCaps = require("./sanitize").caps;
...
function XUnit(options) {
    ...
    this.on("suite:start", function (suite) {
        ...
        var parentSuite = this.findParentSuite(
            suite.parent,
            this.tests[suite.pid][0]
        );
...
XUnit.prototype.genTestsuites = function (tests, pid) {
    ...
    if (options && typeof options.outputDir === "string") {
        this.fileStream = fs.createWriteStream(
            path.join(process.cwd(), options.outputDir, filename)
        );
    }
...
XUnit.prototype.genTestsuite = function (tests) {
    ...
    tests.forEach(function (test) {
        ...
        // Create a custom test case name. This makes it easier to determine which tests failed.
        // Adding in the `test.parent` turns
        // "should look like baseline"
        // into
        // "pfe-tabs should look like baseline"
        const testcaseName = test.parent
        ...
        if (test.pending) {
            ...
            test.err &&
            Object.getOwnPropertyNames(test.err).length > 0
        ) {
            var content = cdata(
                (test.err.stack || "").slice(
                    0,
                    self.errorLogCharacterLimitation
                )
            );
